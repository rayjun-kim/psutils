...
SELECT
  ...
  C.CCSID AS COL_CCSID,
  C2.USER_DEFINED_TYPE_NAME AS UDT_NAME,
  T.SOURCE_TYPE AS UDT_SOURCE_TYPE,
  C2.IS_NULLABLE AS IS_NULLABLE,
  C2.HAS_DEFAULT AS HAS_DEFAULT,
  C2.COLUMN_DEFAULT AS COLUMN_DEFAULT,
  C2.IS_IDENTITY AS IS_IDENTITY,
  C2.IDENTITY_GENERATION AS IDENTITY_GENERATION,
  C2.IDENTITY_START AS IDENTITY_START,
  C2.IDENTITY_INCREMENT AS IDENTITY_INCREMENT,
  C2.IDENTITY_CYCLE AS IDENTITY_CYCLE,
  C2.COLUMN_EXPRESSION AS COLUMN_EXPRESSION,
  COALESCE(C2.LONG_COMMENT, C2.COLUMN_TEXT) AS COLUMN_COMMENT
FROM QSYS2.SYSCOLUMNS C
LEFT JOIN QSYS2.SYSCOLUMNS2 C2 ON C.TABLE_SCHEMA = C2.TABLE_SCHEMA AND C.TABLE_NAME = C2.TABLE_NAME AND C.COLUMN_NAME = C2.COLUMN_NAME
LEFT JOIN QSYS2.SYSTYPES T ON C2.USER_DEFINED_TYPE_SCHEMA = T.USER_DEFINED_TYPE_SCHEMA AND C2.USER_DEFINED_TYPE_NAME = T.USER_DEFINED_TYPE_NAME
WHERE C.TABLE_SCHEMA = '<라이브러리명>' AND C.TABLE_NAME = '<테이블명>'
